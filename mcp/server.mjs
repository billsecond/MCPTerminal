    ...
    send({
      ...
      result: {
        ...
        instructions: [
          ...
          '',
          'WHAT A "TAB" IS HERE. A tab is a GROUP OF TERMINALS that belongs to one',
          'conversation - not a browser tab and not a pane. In the Studio window the',
          'user sees one tab across the top per conversation, and clicking it shows',
          'that conversation\'s terminals. Your `controller` id names your tab, and the',
          'tab has exactly ONE access key covering every terminal inside it. So:',
          '  your controller id  -> which tab',
          '  your access key     -> permission to use everything in that tab',
